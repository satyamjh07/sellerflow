...
const SF = (() => {
  ...
  async function getUser() {
    ...
    const { data, error } = await _supabase
      ...
      .eq("id", uid)
      .maybeSingle();
    // PGRST116 = no rows found (profile deleted but auth session still alive)
    if (error && error.code !== "PGRST116") _throw(error, "getUser");
    return data ? _profileToJS(data) : null;
  }
  ...
  //     any realistic seller order volume.
  ...
  async function _updateCustomerAfterOrder(order) {
    ...
    const { data: cust, error } = await _supabase
      ...
      .single();
    if (error) {
      console.warn("[SF] _updateCustomerAfterOrder fetch failed", error);
      return;
    }
    ...
    const { error: updateErr } = await _supabase
      ...
      .eq("user_id", uid);
    if (updateErr)
      console.warn("[SF] _updateCustomerAfterOrder update failed", updateErr);
  }
