...
// GitHub Pages does not use service workers natively — if one is registered,
// it came from a previous experiment and is likely serving stale JS.
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function _handleSignUp() {
    ...
    try {
      ...
      if (!session) {
        // Email confirmation required — show verification pending screen
        document.getElementById("verify-email-address").textContent = email;
        document.getElementById("resend-email-input").value = email;
        _showScreen("verify-email");
      } else {
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      try {
        ...
        // Cooldown: prevent spam-clicking for 60 seconds
        let seconds = 60;
  ...
  document
    ...
    .addEventListener("click", () => {
      // Pre-fill email from login form if the user already typed it
  ...
  async function _handleForgotPassword() {
    ...
    try {
      ...
      // Show success state — swap form for confirmation message
  ...
  async function _handleResetPassword() {
    ...
    try {
      ...
      // Auto-redirect to dashboard after 2.5s (user is now logged in)
      setTimeout(async () => {
        await _bootApp(Auth.getUser());
      }, 2500);
    } catch (err) {
  ...
  async function _bootApp(authUser) {
    ...
    // ── Step 1: fetch profile for sidebar population ─────────────
    // This is a fast first-pass fetch — used only for the sidebar
    // display name and store name. We re-fetch after Billing.init()
    // below to get the fully up-to-date state for the reminder card.
    ...
    // Show/hide Google provider badge and password change row in settings
    ...
    // ── Step 2: await Billing.init() ──────────────────────────────
    // MUST be awaited before rendering ProfileReminder.
    // Billing.init() sets _currentPlan in memory. If it runs
    // fire-and-forget, Billing.canUploadLogo() returns false during
    // ProfileReminder.render(), causing the logo checklist item to
    // appear incomplete even when it is done.
    try {
      await Billing.init();
    } catch (err) {
      // Non-fatal — app still works on free plan if billing fails
      console.warn("[App] Billing.init non-fatal:", err.message);
    }

    // ── Step 3: re-fetch user AFTER billing init ──────────────────
    // Billing.init() may have called _downgradeToFree() if the plan
    // expired, which writes to the profiles table. We need the fresh
    // state so ProfileReminder reads the correct plan and logoUrl.
    const user = await SF.getUser();

    // ── Step 4: render profile completion reminder ────────────────
  ...
  document
    ...
    .addEventListener("click", async () => {
      ...
      try {
        ...
        // Re-evaluate profile completion reminder with the freshly saved values
  ...
  // Change password link in settings (only shown for non-Google users)
