...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      try {
        ...
        if (isEmailSendError) {
          ...
          smtpFailed = true;
          console.warn("[SignUp] Supabase SMTP failed, will send via EmailJS:", signUpErr.message);
        } else {
      ...
      if (smtpFailed) {
        setTimeout(async () => {
          try {
            ...
            console.warn("[SignUp] Supabase resend backup also failed:", e.message);
          }
        }, 2000);
      }
  ...
  async function _bootApp(authUser) {
    ...
    document.getElementById("sidebar-user-store").textContent      = userForSidebar?.store || "My Store";
