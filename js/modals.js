...
const Modals = (() => {
  ...
  async function saveProduct() {
    ...
    const threshold =
      parseInt(document.getElementById("product-threshold-input").value) || 5;
    const emoji =
      document.getElementById("product-emoji-input").value.trim() || "📦";
  ...
  async function _populateCustomerDropdown() {
    ...
    sel.innerHTML =
      '<option value="">— Select Existing Customer —</option><option value="new">+ New Customer</option>';
    customers.forEach((c) => {
      const opt = document.createElement("option");
      opt.value = c.id;
  ...
  async function addOrderItem() {
    ...
    if (!sel.value) {
      UI.toast("Please select a product", "error");
      return;
    }
  ...
  function renderOrderItems() {
    ...
    if (orderItems.length === 0) {
      el.innerHTML =
        '<div class="text-muted" style="font-size:12px;padding:12px;text-align:center">No items added yet</div>';
      if (totalEl) totalEl.textContent = "₹0";
      return;
    }

    el.innerHTML = orderItems
      .map(
        (item, i) => `
    ...
    try {
      if (customerSel === "new") {
        ...
        const insta = document
          .getElementById("new-customer-insta")
          .value.trim();
        const phone = document
          .getElementById("new-customer-phone")
          .value.trim();
        const email = document
          .getElementById("new-customer-email")
          .value.trim();
        const address = document
          .getElementById("new-customer-address")
          .value.trim();
        const landmark = document
          .getElementById("new-customer-landmark")
          .value.trim();
        const city = document.getElementById("new-customer-city").value.trim();
        const state = document
          .getElementById("new-customer-state")
          .value.trim();
        const pincode = document
          .getElementById("new-customer-pincode")
          .value.trim();

        if (!name) {
          UI.toast("Please enter customer name", "error");
          return;
        }
        ...
        if (Object.keys(updates).length > 0)
          await SF.updateCustomer(nc.id, { ...nc, ...updates });
        ...
        if (!cust) {
          UI.toast("Customer not found", "error");
          return;
        }
        customerId = cust.id;
      ...
      if (customerSel === "new") {
        ...
        const pincode = document
          .getElementById("new-customer-pincode")
          .value.trim();
        var shippingAddress = [addr, lm, city, state, pincode]
          .filter(Boolean)
          .join(", ");
      }
      ...
      setTimeout(async () => {
        try {
          ...
          if (!savedCustomer?.email) {
            console.info(
              `[SellerFlow] Email skipped for ${created.id} — no email on file for customer.`,
            );
            return;
          }

          // Build the full self-contained invoice HTML (email-safe, table-based)
          const invoiceHTML = Components.generateInvoiceEmailHTML(
            created,
            savedCustomer,
            user,
          );
          ...
          UI.toast(
            `Confirmation email sent to ${savedCustomer.email} 📧`,
            "success",
          );
        } catch (emailErr) {
          ...
          UI.toast(
            "Order saved! (Email delivery failed — check browser Console)",
            "warn",
          );
        }
      }, 0);
  ...
  async function showInvoice(id) {
    ...
    try {
      ...
      document.getElementById("invoice-preview-body").innerHTML =
        Components.InvoiceTemplate(o, user || {});
      ...
      toggleBtn.onclick = async () => {
        try {
          ...
          UI.toast(
            `Payment marked as ${isPaid ? "pending" : "paid"}`,
            "success",
          );
          UI.closeModal("modal-invoice");
          await Promise.all([
      ...
      waBtn.title = phoneNum
        ? `Share via WhatsApp to ${phoneNum}`
        : "WhatsApp (No phone saved)";
      waBtn.onclick = () => {
      ...
      // ── Download PDF ────────────────────────────────────────────
      //
      // Why iframe instead of off-screen clone:
      //   html2canvas captures from the viewport origin (0,0). Any element
      //   positioned off-screen (top:-9999px / left:-9999px) is outside the
      //   capture window — the left column gets sliced off. A hidden iframe
      //   has its OWN document and its OWN (0,0) origin, so html2canvas
      //   captures the full width correctly every time.
      //
      const dlBtn = document.getElementById("invoice-download-btn");

      dlBtn.onclick = async () => {
        if (typeof html2pdf === "undefined") {
          UI.toast("PDF library not loaded — please refresh the page", "error");
          return;
        }

        // Loading state
        dlBtn.disabled = true;
        const originalLabel = dlBtn.innerHTML;
        dlBtn.innerHTML =
          '<span style="display:inline-block;width:13px;height:13px;border:2px solid currentColor;border-top-color:transparent;border-radius:50%;animation:spin 0.7s linear infinite;vertical-align:middle;margin-right:6px"></span>Generating…';

        // Create a hidden iframe with its own document — gives html2canvas
        // a real layout context at position (0,0), not off-screen.
        ...
        iframe.style.cssText = [
          ...
          "height:1px", // minimal height — html2pdf expands it
          "border:none",
          "visibility:hidden", // hidden but still laid out
          "z-index:-9999",
        ].join(";");
        ...
        try {
          ...
          // The invoice HTML from Components.InvoiceTemplate is already
          // 100% inline-styled — no external stylesheet needed.
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Ensure table cells never collapse */
...
<body>${invoiceHTML}</body>
          ...
          // Wait for iframe to fully render before capturing
          ...
          // Extra tick to let the browser finish painting
          await new Promise((resolve) => setTimeout(resolve, 100));

          // Get the root element inside the iframe to pass to html2pdf
          const invoiceEl =
            iframeDoc.getElementById("invoice-print-area") ||
            iframeDoc.body.firstElementChild;

          if (!invoiceEl)
            throw new Error("Invoice element not found in iframe");

          // Run html2pdf against the iframe's element.
          // html2canvas will use iframe.contentWindow as its context,
          // so (0,0) is the top-left of the invoice — no cropping.
          const blob = await html2pdf()
            .set({
              ...
              html2canvas: {
                ...
                logging: false,
                // Use the iframe's window so scroll offsets are (0,0)
                windowWidth: 794,
          ...
          // Trigger download — iOS fallback opens in new tab
          ...
          console.error("[SellerFlow] PDF generation failed:", pdfErr);
          UI.toast("PDF generation failed — please try again", "error");
        } finally {
          // Always clean up the iframe and restore button
          document.body.removeChild(iframe);
          dlBtn.disabled = false;
          dlBtn.innerHTML = originalLabel;
        }
      };
    } catch (err) {
  ...
  function _fallbackCopy(text) {
    ...
    try {
      ...
      UI.toast("Message copied!", "success");
    } catch {
